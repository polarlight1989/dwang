...
<div class='itabHeader' style='background-color:#d7d7d7;'>
 ...
 $p = D('Catalogs')->WHERE(array('id'=>$catalog['pid']))->find(); $p['title'] = C('LANG')=='zh'?$p['title']:$p[C('LANG').'title']; echo $p['title']; ?>
		...
		<?php
 $pageLength = 6; $page = I('GET.page',1); $arcs = D('Joinus')->WHERE(array('lang'=>C('LANG')))->page($page,$pageLength)->SELECT(); $count = D('Joinus')->WHERE(array('lang'=>C('LANG')))->count(); ?>
		<!-- 文字列表 -->
 ...
 if($arcs) foreach($arcs as $v){ ?>
				<li>
					<a href="<?php echo arcHref($v[id],41);?>" class="txicon"></a>
					<a href="<?php echo arcHref($v[id],41);?>" class="jltitle1"><?php echo ($v["title"]); ?></a>
					<a href="<?php echo arcHref($v[id],41);?>" class="jltitle2">(<?php echo ($v["intro"]); ?>)</a>
				</li>
			<?php } ?>
			</ul>
