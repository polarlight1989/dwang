...
<div class='itabHeader' style='background-color:#d7d7d7;'>
 ...
 $p = D('Catalogs')->WHERE(array('id'=>$catalog['pid']))->find(); $p['title'] = C('LANG')=='zh'?$p['title']:$p[C('LANG').'title']; echo $p['title']; ?>
		...
		<?php
 $map = array('catalog_id'=>$catalog['id']); $map['lang'] = C('LANG'); $arcs = D('Thumbs')->WHERE($map)->ORDER('CDATE DESC')->select(); $count = count($arcs); ?>
		<!-- 文字列表 -->
		 <div class='itabTxtList il2'  >
			<div class='pdd'>
				<?php
 if($arcs) foreach($arcs as $v){ ?>
				<div class='itcon' style='display:none;width:860px;' aid='<?php echo ($v["id"]); ?>'>
					<div style='height:700px;text-align:center;overflow-y:hidden;'>
					 <img src='<?php echo ($v["thumb"]); ?>' style='margin:15px 0px;'/>
					<p class='it2con' style='line-height:67px;color:#000000;'><img src='/public/images/icon_jiangzhang.png'/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo ($v["title"]); ?></p>
					</div>
				</div>
				<?php } ?>
				<div class='floatChange'>
					<a href='javascript:void(0);' class='jtup'></a>
					<a href='javascript:void(0);' class='jtdown'></a>
					<div class='fcinner' style='525px;'>
						<div class='fnn'>
						<?php
 if($arcs) foreach($arcs as $v){ ?>
						<a href='javascript:void(0)' title='<?php echo ($v["title"]); ?>' onclick='showArc(<?php echo ($v["id"]); ?>)' style='margin-bottom:5px;' ><img src='<?php echo ($v["thumb"]); ?>' height="171px" width="222px"/></a>
				<?php } ?>
						</div>
					</div>
				</div>
		...
		<div class='itabTxtList il22' style='display:none'>
		<?php
 if($arcs) foreach($arcs as $v){ ?>
			<li>
				...
				<div class='itcon'>
					...
					<a href="javascript:showArc(<?php echo ($v["id"]); ?>);" class='it2title'><?php echo ($v["title"]); ?></a>
		...
		var pagenum = count/3+(count%3?1:0);
		var page = 1;
		var bl = 525;
		function toPage(){
			...
			var mr = 0-(page-1)*bl;
			$(".fnn").animate({'margin-top':mr+'px'},300,function(){ 
			});
		}
		$(function(){
