...
<div class='itabHeader' style='background-color:#d7d7d7;'>
 ...
 $p = D('Catalogs')->WHERE(array('id'=>$catalog['pid']))->find(); $p['title'] = C('LANG')=='zh'?$p['title']:$p[C('LANG').'title']; echo $p['title']; ?>
		...
		<?php
 $data = D('Areas')->WHERE(array('lang'=>C('LANG')))->SELECT(); ?>
		<!-- 文字列表 -->
		<div class="itabTxtList il2" style="background-color:#f5f5f5;overflow:hidden;padding:80px 0px;">
			<div class="quyuList">
				...
				<div class="qylBox">
					...
					<ul class="qyaul">
					<?php
 $row = 0; if($data) foreach($data as $v){ ?>
						<li class="<?=$row++==0?'cur':''?>"><?php echo ($v["title"]); ?><i></i></li>
						<?php } ?>
					</ul>
					<?php
 $row = 0; if($data) foreach($data as $v){ $rules = array_filter(explode("\n",$v['rules'])); ?>
				<div class="qybCon" style="<?=$row++==0?'display:nlock':'display:none'?>">
				<?php
 foreach($rules as $rule){ $r = explode("|",$rule); ?>
						<a href="<?=$r[1]?$r[1]:'javascript:void(0);'?>" <?=$r[1]?'target="_blank"':''?> ><?=$r[0]?></a>
						<?php } ?>
					</div>
					<?php } ?> 
 ...
 <script>
$(function(){
	$(".qyaul").children('li').bind('mouseover',function(){
		var name = $(this).html();
		$(".qyaul").children('li').removeClass('cur');
		$(this).addClass('cur');
		var ind = $(this).index();
		$(".qybCon").hide();
		$(".qybCon").eq(ind).show();
