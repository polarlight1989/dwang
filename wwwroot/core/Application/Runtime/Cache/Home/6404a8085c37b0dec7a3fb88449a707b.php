...
<div class='itabHeader' style='background-color:#d7d7d7;'>
 ...
 $p = D('Catalogs')->WHERE(array('id'=>$catalog['pid']))->find(); $p['title'] = C('LANG')=='zh'?$p['title']:$p[C('LANG').'title']; echo $p['title']; ?>
		...
		<?php
 $pagelength = 5; $page = I('GET.page',1); $arcs = D('Leaders')->WHERE(array('lang'=>C('lang')))->page($page,$pagelength)->SELECT(); $count = D('Leaders')->WHERE(array('lang'=>C('lang')))->count(); ?>
		<!-- 文字列表 -->
		<div class='itabTxtList il2' style='background-color:#f5f5f5;overflow:hidden;padding:80px 0px;' >
			<div class='leadersList'>
				<?php
 $row=0; foreach($arcs as $v){ $row++; ?>
				<div class='items' <?php if($row==$pagelength){ ?>style='border-bottom:0px'<?php } ?>>
					<div style='position:relative;'>
					<a href='<?php echo arcHref($v[id],$catalog[id]);?>' class='photo'><img style='height:228px;width:154px' src='<?=$v['thumb']?>'/></a>
					<div class='itemsTxt'>
						<a href='<?php echo arcHref($v[id],$catalog[id]);?>' class='aname'><?php echo ($v["title"]); ?></a>
						<div class='azhiwei'>
							职位：<?php echo ($v["intro"]); ?>
						</div>
						<div class='acontent'>
