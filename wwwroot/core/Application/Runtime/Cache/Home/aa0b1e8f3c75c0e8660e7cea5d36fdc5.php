...
<div class='itabHeader' style='background-color:#d7d7d7;'>
 ...
 $p = D('Catalogs')->WHERE(array('id'=>$catalog['pid']))->find(); $p['title'] = C('LANG')=='zh'?$p['title']:$p[C('LANG').'title']; echo $p['title']; ?>
		...
		<?php
 $map = array('catalog_id'=>$catalog['id']); $map['lang'] = C('LANG'); $arcs = D('Arcs')->WHERE($map)->select(); ?>
		<!-- 文字列表 -->
 ...
 if($arcs) foreach($arcs as $v){ ?>
				<div class='itcon' style='display:none;width:860px;' aid='<?php echo ($v["id"]); ?>'>
					...
					<a href="" class='it2title'><?php echo (msubstr($v["title"],10,0)); ?><br/><?php echo (msubstr($v["title"],80,10)); ?></a>
					...
					<p class='it2con'><?php echo ($v["content"]); ?></p>
					</div>
				</div>
				<?php } ?>
				<div class='floatChange'>
					...
					<div class='fcinner'>
						...
						<?php
 if($arcs) foreach($arcs as $v){ ?>
						<a href='javascript:void(0)' onclick='showArc(<?php echo ($v["id"]); ?>)' ><img src='<?php echo ($v["thumb"]); ?>' height="120px" width="220px"/></a>
				<?php } ?>
						</div>
					</div>
				</div>
