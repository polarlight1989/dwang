...
app.directive('fgInputCheckbox',function($req,$staticData,$timeout){
	return {
		require: "ngModel",
		restrict : 'AEMC',
...
app.directive('fgInputSelect',function($req,$timeout){
	return {
		require: "ngModel",
		restrict : 'AEMC',
		scope : {
		...
		template:'<div class="form-group"> <label class="control-label col-md-3">{{title}}</label> <div class="col-md-4"><select class="bs-select form-control" ng-modal="value">  <option value="{{k}}" ng-repeat="(k,v) in data">{{v}}</option>  </select> </div> </div>',
		link:function($scope,$element,attr,ctrl){ 
