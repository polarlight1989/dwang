...
MetronicApp.config(['$stateProvider', '$urlRouterProvider', function($stateProvider, $urlRouterProvider) {
    ...
    $stateProvider
        ...
        .state('users_list', {
         ...
         .state('users_detail', {
            url: "/users/detail/{id:[0-9]{1,5}}",
            templateUrl: viewurl("users.detail"),
            controller: "users_detail",
        })
		.state('dongtaiDetail', {
        ...
        .state('usersGrouplist', {
            url: "/users/grouplist/",
            templateUrl: viewurl("inc.datatable_page"),
            controller: "usersGrouplist",
        })
        .state('usersGroupDetail', {
            url: "/users/groupdetail/{id:[0-9]{1,4}}",
            templateUrl: viewurl("users.groupDetail"),
            controller: "usersGroupDetail",
        })
        .state('pushsList', {
            url: "/pushs/list/",
            templateUrl: viewurl("inc.datatable_page"),
            controller: "pushsList",
        })
        .state('pushsDetail', {
            url: "/pushs/detail/{id:[0-9]{1,4}}",
            templateUrl: viewurl("pushs.detail"),
            controller: "pushsDetail",
        })
        .state('pushsPush', {
            url: "/pushs/push/{id:[0-9,]{1,9999}}",
            templateUrl: viewurl("pushs.push"),
            controller: "pushsPush",
        })
        .state('pushsData', {
            url: "/pushs/pushdata/",
            templateUrl: viewurl("inc.datatable_page"),
            controller: "pushsData",
        })
        .state('VisitLogs', {
            url: "/Visit/log/",
            templateUrl: viewurl("inc.datatable_page"),
            controller: "VisitLogs",
        })
}]);
...
MetronicApp.run(["$rootScope", "settings", "$state","$staticData", function($rootScope, settings, $state,$staticData) {
    $staticData.init('Pushs');
    $staticData.init('UsersGroup');
    $staticData.init('Catalogs');
	$rootScope.LANG = LANG;
...
var app = MetronicApp;
app.filter('staticValue', ['$staticData', function ($staticData) {  
        return function (id,kname,pk,val) {
            var pk = pk ? pk : 'id';
            var val = val ? val : 'name';
            var data = $staticData.getKV(kname,pk,val);
            data['all'] = '全部';
            var id = id.split(',');
            var result = []; 
            for(var i in id){
